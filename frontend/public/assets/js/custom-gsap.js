...
(function ($) {
  ...
  hoverItems.forEach((item) => {
    const bgElem = item.querySelector(".hover__reveal-bg");
    if (!bgElem) return;

    // Ensure background image is applied from data attribute
    const bgAttr = bgElem.getAttribute("data-background-image");
    if (bgAttr) {
      bgElem.style.backgroundImage = `url('${bgAttr}')`;
    }
    ...
    // Position preview to the right of cursor relative to hover item bounding box
    item.addEventListener("mousemove", (e) => {
      const rect = item.getBoundingClientRect();
      const relX = e.clientX - rect.left;
      const relY = e.clientY - rect.top;

      const w = bgElem.offsetWidth || 360;
      const h = bgElem.offsetHeight || 240;

      // Position 25px to the right of cursor
      let posX = relX + 25;
      if (posX + w > rect.width - 15) {
        posX = relX - w - 20; // flip to left if too close to right edge of container
      }

      // Center vertically relative to cursor position in item
      let posY = relY - (h / 2);

      bgElem.style.transform = `translate3d(${posX}px, ${posY}px, 0)`;
    });
